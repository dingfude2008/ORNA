...
- (void)setPickerStyle:(ZWColorPickerStyle)style {
    ...
    if (self.image == nil) {
        ...
        //将image缩放至和view一样大
        CGSize size = CGSizeMake(self.frame.size.width + 5, self.frame.size.height + 5);
        self.image = [self scaleImage:image toSize:size];
...
- (CGPoint)pointInPie:(CGPoint)point {
    CGPoint center = CGPointMake(self.frame.size.width / 2 + 2.5, self.frame.size.height / 2 + 2.5);
