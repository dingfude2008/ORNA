...
{
    int hour, minute, sencend;
    hour = minute = sencend = 0;
    hour = duration / 3600;
    minute =  (duration - 3600 * hour) / 60;
    sencend = duration - 3600 * hour - minute * 60;
    if (hour) {
        return [NSString stringWithFormat:@"%02d:%02d:%02d", hour, minute, sencend];
    }else{
        return [NSString stringWithFormat:@"%02d:%02d", minute, sencend];
    }
}

+ (NSString *)getDurationType1:(int)duration
{
    int hour, minute, sencend;
    hour = minute = sencend = 0;
    hour = duration / 3600;
    minute =  (duration - 3600 * hour) / 60;
    sencend = duration - 3600 * hour - minute * 60;
    return [NSString stringWithFormat:@"%d:%02d:%02d", hour, minute, sencend];
}
